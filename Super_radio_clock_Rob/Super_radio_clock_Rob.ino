...
float fm_stations[] = {97.9, 91.5, 101.1, 100.7, 102.7, 104.3};
#define MAX_STATIONS (sizeof(fm_stations)/sizeof(float))
...
///digital volume control enable
 ...
 display.print("MHz");
  ...
  else if(mode_set >= 1 && mode_set <= MAX_STATIONS) {
    initial_reception(fm_stations[mode_set - 1]);
    event=0;  
  }
...
void initial_reception(float listen_freq){
    i2c_write(RADIO,0x16,0b00000000,0b00000010);
//        i2c_write(RADIO,0x0F,0b10001000,0b00010000);
    i2c_write(RADIO,0x0F,0b10001000,0b00010000);
///digital volume control enable
